...
**central5** は全検出サイクルを時間順に並べた第4〜第8サイクルを指す（selected10 内の第4〜8番ではない）。
GUI解析結果サマリは central5 指標を中心にセクション見出し付きで表示する。
summary.csv には central5 系指標を英語列名で出力する（`cycle_time_mean_s_central5` 等）。
振幅（`amp_mean_px_central5` / `amp_sd_px_central5`）は summary.csv には残るが GUI には表示しない。

### selected10 指標（補助・後方互換）

selected10 はサイクル時間 CV が最小の連続10サイクル。以下の指標は後方互換性・補助確認用として summary.csv に残すが、GUI主表示からは外れている。
...
| 解析結果サマリ | `summary.csv` の主要指標を解析完了後にセクション見出し付きで表示。central5 指標を中心に構成し、平均値と標準偏差は「平均値 ± 標準偏差」形式で1行表示 |
